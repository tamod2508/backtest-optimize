...
class Nsga2:
    def __init__(self, exchange: str, symbol: str, strategy: str, tf: str, from_time: int, to_time: int,
        ...
        self.population_params = []  #Used to store params so that the offspring population does not have the same params as the parent population

        if self.strategy in ["obv", "ichimoku", "sup_res", "macd"]:  #Data Extraction
    ...
    def _params_constraints(self, params: typing.Dict) -> typing.Dict:
        ...
        """
        Method to check that the random parameters created are logical according to the indicator used
        NEEDS TO BE EDITED WHILE ADDING NEW STRATEGY TO THE OPTIMIZER
        NOTE: Be careful that the original dataset used does not get modified as it will result in wrong backtests
        :param params: Backtests with random strategy parameters
        :return: Strategy with logical strategy parameters
        """
        ...
        if self.strategy == "obv":
            ...
            params["macd_slow"] = max(params["macd_slow"], params["macd_fast"])
    ...
    def create_initial_population(self) -> typing.List[BacktestResult]:
        ...
        """
        Creates the initial population of backtests with random parameters to generate the offspring populations
        DO NOT EDIT
        :return: List of Initial population of backtests
        """
        ...
        while len(population) < self.population_size:
            ...
            for p_code, p in self.params_data.items():   #Checks that the random parameters generated fall between the maximum and minimum as specified in the utils module
                if p["type"] == int:
    ...
    def evaluate_population(self, population: typing.List[BacktestResult]) -> typing.List[BacktestResult]:
        ...
        """
        Performs the backtest for each individial in the population and gets the KPIs
        NEEDS TO BE MODIFIED WHILE ADDING NEW STRATEGY and NEW KPI
        :param population: Lists of Backtest Populations to be evaluated
        :return: Lists of backtests with their KPIs evaluated
        """
        ...
        if self.strategy == "obv":
            ...
            for bt in population:
                ...
                if bt.pnl == 0:  #Multi objective optimization might result in 0 pnl for another ooptimized parameter,so we remove them
                    bt.pnl = -float("inf")
                    bt.max_dd = float("inf")
            ...
            for bt in population:
                bt.pnl, bt.max_dd = strategies.macd.backtest(self.data,
                                                                           macd_fast=bt.parameters["macd_fast"],
                                                                           macd_slow=bt.parameters[
                                                                               "macd_slow"],
    ...
    def crowding_distance(self, population: typing.List[BacktestResult]) -> typing.List[BacktestResult]:
        ...
        """
        Calculates the crowding distance of each evaluated member
        NEEDS to be edited while adding more objectives for optimization
        :param population:
        :return: crowding distance for each memeber of the ppopulation
        """
        ...
        for objective in ["pnl", "max_dd"]:
            ...
            population[0].crowding_distance = float("inf") #Sets the max and min crowding distance as inf
    ...
    def non_dominated_sorting(self, population: typing.Dict[int, BacktestResult]) -> typing.List[typing.List[BacktestResult]]:
        ...
        """
        Non dominated sorting is performed according to the KPIs to be optimized
        EDIT WHEN ADDING NEW KPI FOR OPTIMIZATION(refer to example multi objective)
        :param population: List of populations to be sorted(Parent and offspring)
        :return: List of backtests sorted in fronts according to kpis
        """

        fronts = []

        for id_1, indiv_1 in population.items():  #calcuating members of the first front
        ...
        while True:
            ...
            for indiv_1 in fronts[i]:
                for indiv_2_id in indiv_1.dominates:
                    population[indiv_2_id].dominated_by -= 1 #For the generation of fronts after rank 1 ,the domimated by attribute is reduced to zero and process loops
            ...
            if len(fronts[i + 1]) > 0:
                ...
                del fronts[-1]  #deletes the last front as it has no members as all memebers are sorted
                break
    ...
    def create_offspring_population(self, population: typing.List[BacktestResult]) -> typing.List[BacktestResult]:
        ...
        """
        Generates the offspring ppopulation from the initial parent populations and the new generations
        :param population: Parent Pouplation for each generation
        DO NOT EDIT
        :return: List of offspring populations
        """
        ...
        while len(offspring_pop) != self.population_size:
            ...
            for i in range(2):  #Selecting 2 random best fit parents from the parent population
            ...
            for p in params_to_change:
                ...
                new_child.parameters[p] = max(new_child.parameters[p], self.params_data[p]["min"])  #Makes sure that the population due to mutation does not havr param that fall outside the min and max as specified in the utils module
                new_child.parameters[p] = min(new_child.parameters[p], self.params_data[p]["max"])

                if self.params_data[p]["type"] == float: #rounds the random params generated to the decimals as specified in uitls
    ...
    def create_new_population(self, fronts: typing.List[typing.List[BacktestResult]]) -> typing.List[BacktestResult]:
        ...
        """
        Creates a new poplations from the fronts using the best fit parents
        DO NOT EDIT
        :param fronts: sorted list of Parent population
